...
from wheel_hold_task.wheel_hold_env_pid import WheelHoldEnvPID


@configclass
class WheelHoldPIDPPOCfg(RslRlOnPolicyRunnerCfg):
    num_steps_per_env = 24
    max_iterations = 1500
    save_interval = 100
    experiment_name = "wheel_hold_pid"
...
env = WheelHoldEnvPID(env_cfg)
env = RslRlVecEnvWrapper(env)
agent_cfg = WheelHoldPIDPPOCfg()
agent_cfg.device = "cuda:0"
log_dir = os.path.join("logs", "wheel_hold_pid", datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
