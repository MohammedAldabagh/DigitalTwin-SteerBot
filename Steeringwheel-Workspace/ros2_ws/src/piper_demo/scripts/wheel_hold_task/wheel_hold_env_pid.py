...
class WheelHoldEnvPID(DirectRLEnv):
    cfg: WheelHoldEnvCfg
    ...
    def __init__(self, cfg: WheelHoldEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.pid_integral = torch.zeros(self.num_envs, device=self.device)
        self.pid_prev_error_deg = torch.zeros(self.num_envs, device=self.device)
        self.last_gains = torch.zeros((self.num_envs, 3), device=self.device)
        self.last_correction_deg = torch.zeros(self.num_envs, device=self.device)
    ...
    def _decode_gains(self, raw_action: torch.Tensor) -> torch.Tensor:
        a = raw_action.clamp(-1.0, 1.0)
        a01 = (a + 1.0) * 0.5
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        self.last_gains = self._decode_gains(actions)
    ...
    def _apply_action(self) -> None:
        ...
        wheel_pos_rad = self._get_wheel_angle_rad()
        err_rad = self.wheel_target_rad - wheel_pos_rad
        err_deg = torch.rad2deg(err_rad)

        kp, ki, kd = self.last_gains[:, 0], self.last_gains[:, 1], self.last_gains[:, 2]

        large_err = torch.abs(err_deg) > 15.0
        self.pid_integral = torch.where(large_err, torch.zeros_like(self.pid_integral), self.pid_integral + err_deg * self.step_dt)
        self.pid_integral = torch.clamp(self.pid_integral, -INTEGRAL_CLAMP, INTEGRAL_CLAMP)

        deriv_deg = (err_deg - self.pid_prev_error_deg) / self.step_dt

        correction_deg = kp * err_deg + ki * self.pid_integral + kd * deriv_deg
        correction_deg = torch.clamp(correction_deg, -MAX_CORRECTION_DEG, MAX_CORRECTION_DEG)
        self.last_correction_deg = correction_deg
        self.pid_prev_error_deg = err_deg
        ...
        arc_delta = torch.deg2rad(correction_deg) * self.rim_radius
    ...
    def _release_wheel_drive_if_grasped(self):
        ...
        self.pid_prev_error_deg[env_ids] = torch.rad2deg(self.wheel_target_rad[env_ids] - base_angle)
    ...
    def _get_observations(self) -> dict:
        ...
        obs = torch.cat((
            ...
            (self.pid_integral / INTEGRAL_CLAMP).unsqueeze(-1),
        ), dim=-1)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        correction_penalty = torch.square(self.last_correction_deg / MAX_CORRECTION_DEG)
        arm_pos = self.robot.data.joint_pos[:, self._arm_dof_idx]
        ...
        hold_reward = (
            ...
            + self.cfg.rew_scale_action_penalty * correction_penalty
            + alive_bonus
        )
        ...
        self.extras["log"] = {
            ...
            "correction_deg": self.last_correction_deg.mean(),
            "kp": self.last_gains[:, 0].mean(),
            "ki": self.last_gains[:, 1].mean(),
            "kd": self.last_gains[:, 2].mean(),
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        self.pid_integral[env_ids] = 0.0
        self.pid_prev_error_deg[env_ids] = 0.0
        self.last_gains[env_ids] = 0.0
        self.last_correction_deg[env_ids] = 0.0
