...
from wheel_hold_task.wheel_hold_env_v2 import WheelHoldEnvV2

@configclass
class WheelHoldV2PPOCfg(RslRlOnPolicyRunnerCfg):
    num_steps_per_env = 24
    max_iterations = 1500
    save_interval = 100
    experiment_name = "wheel_hold_v2"
...
env_cfg.action_space = 1
env_cfg.observation_space = 15
env = WheelHoldEnvV2(env_cfg)
env = RslRlVecEnvWrapper(env)
agent_cfg = WheelHoldV2PPOCfg()
agent_cfg.device = "cuda:0"
log_dir = os.path.join("logs", "wheel_hold_v2", datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
